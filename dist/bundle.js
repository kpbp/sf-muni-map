...
// Credit to https://davidwalsh.name/convert-xml-json
...
class BusTooltip {
    ...
    show (bus) {
        this.tooltip
            ...
            .style("left", (event.pageX - TOOLTIP_SIZE / 2) + "px")
            .style("top", (event.pageY - TOOLTIP_SIZE - OFFSET) + "px");
    }
...
class BusController {
    ...
    update () {
        fetchXMLAsJSON(this.dataUrl)
            .then(json$$1 => json$$1.body.vehicle)
...
class BusTogglePaneController {

    constructor({ container, onToggle=()=>{} }) {
        this.onToggle = onToggle;
        this.toggles = select(container)
            .append('ul')
            .attr('class', 'toggles');
        this.routeMap = {};
    }
    ...
    setRoutes (buses) {
        ...
        buses.forEach(({ routeTag, color: color$$1 }) => {
    ...
    handleToggle (clickedRoute, _index, nodes) {
        let routes = selectAll(nodes).data();
...
class App {
    ...
    constructor (config = {}) {
        ...
        let svg = select(this.config.mapContainer)
            .append('svg')
